...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		Vec3f a = Vec3f(v0.position.x - v1.position.x, v0.position.y - v1.position.y, v0.position.z - v1.position.z);
		Vec3f b = Vec3f(v1.position.x - v2.position.x, v1.position.y - v2.position.y, v1.position.z - v2.position.z);
		v0.normal = b.cross(a).normalized();
		v1.normal = v0.normal;
		v2.normal = v0.normal;
...
vector<Vertex> App::loadObjFileModel(string filename) {
	...
	while(getline(input, line)) {
		...
		if (s == "v") { // vertex position
			...
			v.x = std::stof(line.substr(ptr), &sz);
			ptr += sz;
			v.y = std::stof(line.substr(ptr), &sz);
			ptr += sz;
			v.z = std::stof(line.substr(ptr));
			...
			f[0] = std::stoi(line.substr(ptr), &sz) -1;
			ptr += sz;
			sink = std::stoi(line.substr(ptr), &sz);
			ptr += sz;
			f[1] = std::stoi(line.substr(ptr), &sz) - 1;
			ptr += sz;

			f[2] = std::stoi(line.substr(ptr), &sz) - 1;
			ptr += sz;
			sink = std::stoi(line.substr(ptr), &sz);
			ptr += sz;
			f[3] = std::stoi(line.substr(ptr), &sz) - 1;
			ptr += sz;

			f[4] = std::stoi(line.substr(ptr), &sz) - 1 ;
			ptr += sz;
			sink = std::stoi(line.substr(ptr), &sz);
			ptr += sz;
			f[5] = std::stoi(line.substr(ptr)) - 1;
